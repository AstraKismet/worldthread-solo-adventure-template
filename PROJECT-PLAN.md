...
## 階段 7：`0.6.x` 收斂（2026-07-19 決策看板 12 題定案；先不進 `0.7.0`）

前置研究＝0.7.0 方向研究報告（本機 `WORLDTHREAD-ROADMAP-RESEARCH.local.md`，2026-07-18，內外部九研究 lane＋對抗查證）。核心結論：`0.6.0` 四項新機制（短團收尾、共窗多人 Hot Seat、host-log、campaign-arc）零實測資料點、其中 `campaign_status` 已是可視化衍生專案的讀取契約——本階段專注把 `0.6.0` 做實；功能面擴充維持訊號驅動延後（〈後續擴展〉backlog 不動）。決策看板 12 題（A1–A3 補測／B1–B6 條文包／C1–C2 文件債／D1–D4 其他）全採建議定案。
...
- [ ] **文件債（C1／C2／D3＋版本行修正）**：DESIGN 狀態列版本字串修正（原誤留「`0.5.0` 已發行」——`0.6.0` 收版時漏改版本本體的事實錯誤）；DESIGN §6 補戰役收尾指路句＋§8 補收尾驗收句（C1 最小組合，指路寫法防雙準源）；PLAN 狀態列結構化表格試點（C2，本檔）；Claude 專屬 rules 檔補 fable 不可用時架構設計 fallback 句（D3）。
