...
1. **程式擲骰（優先）**：依你所在環境的執行能力擇一呼叫，並在回覆中逐字引用其輸出的 JSON 行——能執行 Node 18+ 用 `node tools/dice.mjs 2d6+3`；能執行 Python 3.8+ 用 `python tools/dice.py 2d6+3`。兩支工具輸出契約完全相同，任選其一即可。
